...
// if (!(typeof globalThis !== "undefined" && globalThis.randomScenarioBatchQuiet)) {
   console.log("START HYBRID KINEMATICS");
// } // START log guard
...
-
    ...
    STEP_DOWN_XP -> CLIMB_UP_XP
    STEP_DOWN_XN -> CLIMB_UP_XN
    STEP_DOWN_ZP -> CLIMB_UP_ZP
    STEP_DOWN_ZN -> CLIMB_UP_ZN
    
    STEP_UP_XN -> CLIMB_DOWN_XN
    STEP_UP_XP -> CLIMB_DOWN_XP
    STEP_UP_ZN -> CLIMB_DOWN_ZN
    STEP_UP_ZP -> CLIMB_DOWN_ZP
...
*/
      ...
      
 {
  ...
  const maxDebugRejections = Math.max(0, Number(options.max_debug_rejections ?? 5000));
  ...
  const buildFinalResult = (pathData, extra = {}) => {
    ...
    const result = {
      ok: Boolean(extra.ok),
      answer: "calc_hybrid_kinematics_path",
...
/*
console.log("result.path_found:", result.path_found, "states:", result.states?.length, "actions:", result.actions?.length);
if (result.actions?.length > 0) {
  console.log("result.actions:", JSON.stringify(result.actions));
} else {
  console.log("result.actions: EMPTY - path_found=" + result.path_found + " states=" + result.states?.length);
}
*/
    ...
    if (!extra.ok && options.debug_log === true) {
      ...
      const bestPartialData = bp ? {
        ...
        actions: bp.actions ?? [],
        terminal_state: decoratePathDataForOutput(bp)?.terminal_state ?? null,
        total_cost: bp.total_cost ?? 0,
      } : {
        states: publicPathData?.states ?? [],
        actions: publicPathData?.actions ?? [],
      ...
      const pathAnalysis = (() => {
        ...
        const bpActions = bestPartialData.actions ?? [];
        for (let i = 0; i < bpStates.length; i++) {
          const s = bpStates[i];
          const action = i > 0 && i <= bpActions.length ? bpActions[i - 1] : null;
          steps.push({
            step: i,
            pos: [s.x, s.y, s.z],
            dir: s.dir,
            support: s.support_mode,
            action: action,
          });
        }

        // Distanz terminal -> goal
        const ts = bestPartialData.terminal_state;
        const gs = publicGoalState;
        let distInfo = null;
        if (ts && gs) {
          const dx = Number(gs.x) - Number(ts.x);
          const dy = Number(gs.y) - Number(ts.y);
          const dz = Number(gs.z) - Number(ts.z);
          distInfo = {
            terminal: [ts.x, ts.y, ts.z],
            goal: [gs.x, gs.y, gs.z],
            delta: [dx, dy, dz],
        ...
        // Blockierte Primitiven vom terminal state aus (aus debug_rejections)
        const blockedFromTerminal = [];
        if (ts) {
          for (const r of debugRejections) {
            if (!r.from) continue;
            if (Number(r.from.x) === Number(ts.x) && Number(r.from.y) === Number(ts.y) && Number(r.from.z) === Number(ts.z)) {
              blockedFromTerminal.push({
                primitive: r.primitive,
                error: r.error,
                target: r.target ? [r.target.x, r.target.y, r.target.z] : null,
                contact: r.contact ? [r.contact.x, r.contact.y, r.contact.z] : null,
              });
            }
          }
        }
        ...
        return {
          steps: steps,
          distance_to_goal: distInfo,
          blocked_from_terminal: blockedFromTerminal,
        };
      })();
      ...
      const logData = {
        ...
        start_state: publicStartState,
        goal_state: publicGoalState,
        best_partial: bestPartialData,
        path_analysis: pathAnalysis,
        debug_rejections: debugRejections,
      };
      try {
        fs.writeFileSync(logFile, JSON.stringify(logData, null, 2));
        console.log("DEBUG: hybrid path debug log written to " + logFile);
      } catch (e) {
        console.error("DEBUG: failed to write hybrid path debug log:", e.message);
      }

      // --- menschenlesbare .txt Log-Datei ---
      const txtFile = logDir + '/hybrid_path_debug.txt';
      const txtLines = [];
      txtLines.push('=== HYBRID PATH DEBUG ===');
      txtLines.push('Timestamp: ' + new Date().toISOString());
      txtLines.push('Start: (' + publicStartState.x + ',' + publicStartState.y + ',' + publicStartState.z + ') ' + publicStartState.dir);
      txtLines.push('Goal:  (' + publicGoalState.x + ',' + publicGoalState.y + ',' + publicGoalState.z + ') ' + publicGoalState.dir);
      txtLines.push('Error: ' + (extra.error ?? 'PATH_NOT_FOUND'));
      txtLines.push('Expanded Nodes: ' + expandedNodes);
      txtLines.push('Generated Nodes: ' + generatedNodes);
      txtLines.push('Dominant Block Reason: ' + dominantBlockReason);
      txtLines.push('');

      const bpStates = bestPartialData.states ?? [];
      const bpActions = bestPartialData.actions ?? [];
      txtLines.push('=== BEST PARTIAL PATH (' + bpStates.length + ' states, cost=' + bestPartialData.total_cost + ') ===');
      for (let i = 0; i < bpStates.length; i++) {
        const s = bpStates[i];
        const action = i > 0 && i <= bpActions.length ? bpActions[i - 1] : null;
        const actionStr = action ? '  \u2190 ' + action : '';
        const marker = (i === bpStates.length - 1) ? '  <-- TERMINAL' : '';
        txtLines.push(
          String(i).padStart(3) + ': (' +
          String(s.x).padStart(2) + ',' +
          String(s.y).padStart(2) + ',' +
          String(s.z).padStart(2) + ') ' +
          (s.dir ?? '?').padEnd(3) +
          actionStr + marker
        );
      }
      ...
      if (ts && gs) {
        ...
        const dz = Number(gs.z) - Number(ts.z);
        txtLines.push('');
        txtLines.push('=== DISTANCE TO GOAL ===');
        txtLines.push('  Terminal: (' + ts.x + ',' + ts.y + ',' + ts.z + ')');
        txtLines.push('  Goal:     (' + gs.x + ',' + gs.y + ',' + gs.z + ')');
        txtLines.push('  Delta:    [' + dx + ',' + dy + ',' + dz + ']');
        txtLines.push('  Manhattan: ' + (Math.abs(dx) + Math.abs(dy) + Math.abs(dz)));
      }

      // Error stats
      txtLines.push('');
      txtLines.push('=== ERROR STATS ===');
      for (const [reason, count] of Object.entries(errorStats)) {
        txtLines.push('  ' + reason + ': ' + count);
      }

      try {
        fs.writeFileSync(txtFile, txtLines.join('\n') + '\n');
        console.log("DEBUG: hybrid path debug txt written to " + txtFile);
      } catch (e) {
        console.error("DEBUG: failed to write hybrid path debug txt:", e.message);
      }
    }
