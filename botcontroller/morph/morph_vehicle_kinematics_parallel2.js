...
class MorphVehicleKinematicsParallel2 extends MorphBase
{
    ...
    constructor(startBots, targetBots, params)
    {
        ...
        this.debugLogPath = path.join(__dirname, 'morph_vehicle_kinematics_parallel2.log');
        fs.writeFileSync(this.debugLogPath, '', 'utf8');
        this.morphPlanLogPath = path.join(__dirname, 'morph_vehicle_kinematics_parallel2_morphplan.txt');
        ...
        this.validPathLogPath = path.join(__dirname, 'morph_vehicle_kinematics_parallel2_validpath.log');
    ...
    // Checks if a bot is a surplus candidate: inside emptyArea but NOT at a target position.
    // Surplus candidates should be prioritized as donors for free targets inside the emptyArea,
    // before importing bots from outside the emptyArea.
    // Returns false if no emptyArea is defined (normal morph).
    //
    isSurplusCandidate(bot)
    {
        const emptyArea = this.params?.emptyArea;
        if (!emptyArea || emptyArea.x === undefined) return false;
        const x = Number(bot.x), y = Number(bot.y), z = Number(bot.z);
        // Check if bot is inside emptyArea
        if (x < emptyArea.x || x > emptyArea.x2 ||
            y < emptyArea.y || y > emptyArea.y2 ||
            z < emptyArea.z || z > emptyArea.z2) return false;
        // Check if bot is NOT at a target position
        const targets = this.getAllBots(this.cluster_target);
        return !targets.some(t => Number(t.x) === x && Number(t.y) === y && Number(t.z) === z);
    } // isSurplusCandidate()
    ...
    choosePair(collection = this.cells, attemptedPairs = new Set(), reservedTargets = new Set(), movedBotsInWave = new Set(), skippedBotsInWave = new Set(), pathResult = null, forbiddenCells = null)
    {
        ...
        for (const c of botCandidates) {
            ...
            const below = this.getAllBots(collection).find(b =>
                b.x === c.bot.x && b.y === c.bot.y - 1 && b.z === c.bot.z
            );
            c.hasFreeYPlus = !above;
        ...
        botCandidates.sort((a, b) => {
            ...
            if (!aIsReserve && bIsReserve) return 1;
            // Surplus candidates (inside emptyArea, NOT at target) get priority
            // over external bots. This ensures existing bots inside the emptyArea
            // are used as donors before importing new bots from outside.
            const aIsSurplus = this.isSurplusCandidate(a.bot);
...
*/
      ...
      
 {
    ...
    stepMorph(caller, finishHandler) {
        ...
        let plannedCells;
        try {
            plannedCells = JSON.parse(JSON.stringify(this.cells));
        } catch (e) {
            this.debugLog(`[DEBUG stepMorph] ERROR copying cells: ${e.message}`);
            this.log(`ERROR in stepMorph: ${e.message}`);
            finishHandler(this.morphLog, false);
            return;
        }
        this.debugLog(`[DEBUG stepMorph] cells copied: ${Object.keys(plannedCells).length || plannedCells.length} bots`);

        // ─────────────────────────────────────────────────────────────────
        // SURPLUS BOT HANDLING (emptyArea): drain surplus bots out first
        // MUST run before areAllBotsHappy check, otherwise surplus bots inside
        // emptyArea are ignored when all target positions are already occupied.
        // ─────────────────────────────────────────────────────────────────
        const emptyArea = this.params?.emptyArea;
        if (emptyArea && emptyArea.x !== undefined) {
            const startBots = this.getAllBots(plannedCells);
            const targetBots = this.getAllBots(this.cluster_target);
            const isInside = (bot) => {
                const x = Number(bot.x), y = Number(bot.y), z = Number(bot.z);
                return x >= emptyArea.x && x <= emptyArea.x2 &&
                       y >= emptyArea.y && y <= emptyArea.y2 &&
                       z >= emptyArea.z && z <= emptyArea.z2;
            };
            const startInside = startBots.filter(isInside);
            const targetInside = targetBots.filter(isInside);
            const surplusCount = Math.max(0, startInside.length - targetBots.length);

            // ── Add cleanup targets for surplus bots ──
            // Find free positions outside emptyArea with cluster contact
            // and add them to cluster_target so choosePair moves surplus bots there.
            if (surplusCount > 0) {
                const targetPosSetLocal = new Set(targetBots.map(t => `${t.x},${t.y},${t.z}`));
                const occupiedSet = new Set();
                for (const b of startBots) occupiedSet.add(`${b.x},${b.y},${b.z}`);
                for (const t of targetBots) occupiedSet.add(`${t.x},${t.y},${t.z}`);

                const cleanupTargets = [];
                for (const surplusBot of startInside) {
                    if (cleanupTargets.length >= surplusCount) break;
                    if (targetPosSetLocal.has(`${surplusBot.x},${surplusBot.y},${surplusBot.z}`)) continue;
                    // Find a free position outside emptyArea with cluster contact
                    let foundPos = null;
                    for (let r = 1; r <= 8 && !foundPos; r++) {
                        for (let dx = -r; dx <= r && !foundPos; dx++) {
                            for (let dy = -r; dy <= r && !foundPos; dy++) {
                                for (let dz = -r; dz <= r && !foundPos; dz++) {
                                    const cx = Number(surplusBot.x) + dx;
                                    const cy = Number(surplusBot.y) + dy;
                                    const cz = Number(surplusBot.z) + dz;
                                    if (cx >= emptyArea.x && cx <= emptyArea.x2 &&
                                        cy >= emptyArea.y && cy <= emptyArea.y2 &&
                                        cz >= emptyArea.z && cz <= emptyArea.z2) continue;
                                    if (occupiedSet.has(`${cx},${cy},${cz}`)) continue;
                                    if (!this.hasContact(cx, cy, cz, plannedCells)) continue;
                                    foundPos = { x: cx, y: cy, z: cz };
                                }
                            }
                        }
                    }
                    if (foundPos) {
                        cleanupTargets.push(foundPos);
                        occupiedSet.add(`${foundPos.x},${foundPos.y},${foundPos.z}`);
                    }
                }

                if (cleanupTargets.length > 0) {
                    this.debugLog(`[SURPLUS_TARGETS] Adding ${cleanupTargets.length} cleanup target(s) to cluster_target:`);
                    for (const ct of cleanupTargets) {
                        this.debugLog(`[SURPLUS_TARGETS]   (${ct.x},${ct.y},${ct.z})`);
                        // Add to cluster_target as a virtual target
                        this.cluster_target.push({ x: ct.x, y: ct.y, z: ct.z });
                    }
                    this.debugLog(`[SURPLUS_TARGETS] cluster_target now has ${this.cluster_target.length} target(s).`);
                }
            }

                        if (surplusCount > 0) {
                // Sort surplus candidates: prefer bots NOT in target positions
                const targetPosSet = new Set(targetBots.map(t => `${t.x},${t.y},${t.z}`));
                startInside.sort((a, b) => {
                    const aInTarget = targetPosSet.has(`${a.x},${a.y},${a.z}`);
                    const bInTarget = targetPosSet.has(`${b.x},${b.y},${b.z}`);
                    return (aInTarget ? 1 : 0) - (bInTarget ? 1 : 0);
                });

                const surplusWaveMoves = [];
                let addedSurplus = 0;
                for (const surplusBot of startInside) {
                    if (addedSurplus >= surplusCount) break;
                    const key = `${surplusBot.x},${surplusBot.y},${surplusBot.z}`;
                    if (targetPosSet.has(key)) continue;

                    // Find nearest free position outside emptyArea
                    const occupied = new Set();
                    for (const b of startBots) occupied.add(`${b.x},${b.y},${b.z}`);
                    for (const t of this.cluster_target) occupied.add(`${t.x},${t.y},${t.z}`);

                    let foundPos = null;
                    // Scan positions directly outside the emptyArea boundary first.
                    // Same y-level positions are preferred (most reachable for VK movement).
                    const directions = [
                        { dx: 1, dy: 0, dz: 0 }, { dx: -1, dy: 0, dz: 0 },
                        { dx: 0, dy: 1, dz: 0 }, { dx: 0, dy: -1, dz: 0 },
                        { dx: 0, dy: 0, dz: 1 }, { dx: 0, dy: 0, dz: -1 }
                    ];
                    // Scan up to 5 steps in each direction for positions outside the emptyArea
                    for (const dir of directions) {
                        for (let step = 1; step <= 5 && !foundPos; step++) {
                        const cx = Number(surplusBot.x) + dir.dx * step;
                        const cy = Number(surplusBot.y) + dir.dy * step;
                        const cz = Number(surplusBot.z) + dir.dz * step;
                        if (cx >= emptyArea.x && cx <= emptyArea.x2 &&
                            cy >= emptyArea.y && cy <= emptyArea.y2 &&
                            cz >= emptyArea.z && cz <= emptyArea.z2) continue;
                        if (occupied.has(`${cx},${cy},${cz}`)) continue;
                        if (cx < -5 || cx > 15 || cy < -5 || cy > 15 || cz < -5 || cz > 15) continue;
                        foundPos = { x: cx, y: cy, z: cz };
                        }  // step loop
                        break;
                    }
                    // Fallback: scan all levels, preferring same y
                    for (let radius = 1; radius <= 8 && !foundPos; radius++) {
                        for (let dx = -radius; dx <= radius && !foundPos; dx++) {
                            for (let dy = -radius; dy <= radius && !foundPos; dy++) {
                                for (let dz = -radius; dz <= radius && !foundPos; dz++) {
                                    const cx = Number(surplusBot.x) + dx;
                                    const cy = Number(surplusBot.y) + dy;
                                    const cz = Number(surplusBot.z) + dz;
                                    if (cx >= emptyArea.x && cx <= emptyArea.x2 &&
                    ...
                    if (!foundPos) {
                        this.debugLog(`[EMPTYAREA] Could not find position for surplus bot ${surplusBot.id}`);
                        continue;
                    }

                    // Try multiple candidate positions: collect ALL free positions outside emptyArea,
                    // then iterate through them until pathfinding succeeds or all are exhausted.
                    const surplusCandidates = [];
                    // Collect all candidates found during scanning (including foundPos as first)
                    surplusCandidates.push(foundPos);
                    // Re-scan to collect more candidates (up to 5)
                    for (let r = 1; r <= 5; r++) {
                        for (let dx = -r; dx <= r && surplusCandidates.length < 10; dx++) {
                            for (let dy = -r; dy <= r && surplusCandidates.length < 10; dy++) {
                                for (let dz = -r; dz <= r && surplusCandidates.length < 10; dz++) {
                                    const cx = Number(surplusBot.x) + dx;
                                    const cy = Number(surplusBot.y) + dy;
                                    const cz = Number(surplusBot.z) + dz;
                                    if (cx >= emptyArea.x && cx <= emptyArea.x2 &&
                                    ...
                                    // Avoid duplicates
                                    const ck = `${cx},${cy},${cz}`;
                                    if (!surplusCandidates.some(c => c.x === cx && c.y === cy && c.z === cz)) {
                                        surplusCandidates.push({ x: cx, y: cy, z: cz });
                                    }
                                }
                            }
                        }
                    ...
                    // Build path: surplus bot → outside position
                    const surplusStart = { x: Number(surplusBot.x), y: Number(surplusBot.y), z: Number(surplusBot.z),
                                           vx: Number(surplusBot.vector_x ?? 0), vy: Number(surplusBot.vector_y ?? 0), vz: Number(surplusBot.vector_z ?? 1) };
                    let pathFound = false;
                    for (const candidate of surplusCandidates) {
                        if (pathFound) break;
                        this.debugLog(`[EMPTYAREA] Surplus candidate: ${surplusBot.id} @ (${surplusBot.x},${surplusBot.y},${surplusBot.z}) → target (${candidate.x},${candidate.y},${candidate.z})`);
                        const surplusTarget = { x: candidate.x, y: candidate.y, z: candidate.z };
                        const pathResult = {};
                        const planOk = this._buildWorldAndPlanPath(surplusStart, surplusTarget, plannedCells, surplusBot.id, null, pathResult);

                    if (planOk && pathResult.vkResult && pathResult.vkResult.ok) {
                        // Use this candidate - found a valid path!
                        const foundPos = candidate;
                        const vk = pathResult.vkResult;
                        const coordPath = vk.states.map(s => [Number(s.x), Number(s.y), Number(s.z)]);
                        const targetVx = Number(vk.goal_state?.vx ?? surplusStart.vx);
                        const targetVy = Number(vk.goal_state?.vy ?? 0);
                        const targetVz = Number(vk.goal_state?.vz ?? 1);

                        surplusWaveMoves.push({
                            path: coordPath,
                            botStart: surplusStart,
                            botTarget: { x: foundPos.x, y: foundPos.y, z: foundPos.z, vx: targetVx, vy: targetVy, vz: targetVz },
                            botId: surplusBot.id,
                            pathStates: vk.states
                        });

                        this.addBotToCells({
                            id: surplusBot.id,
                            x: foundPos.x, y: foundPos.y, z: foundPos.z,
                            vx: targetVx, vy: targetVy, vz: targetVz
                        }, plannedCells);

                        occupied.add(`${foundPos.x},${foundPos.y},${foundPos.z}`);
                        this.debugLog(`[EMPTYAREA] Surplus wave: ${surplusBot.id} @ (${surplusBot.x},${surplusBot.y},${surplusBot.z}) → outside (${foundPos.x},${foundPos.y},${foundPos.z})`);
                        addedSurplus++;
                        pathFound = true;
                    } else {
                        this.debugLog(`[EMPTYAREA] No path for surplus bot ${surplusBot.id} → (${candidate.x},${candidate.y},${candidate.z})`);
                    } // if planOk
                } // for candidate
                if (!pathFound) {
                    this.debugLog(`[EMPTYAREA] All ${surplusCandidates.length} candidates exhausted for surplus bot ${surplusBot.id} – cannot move out! Continuing morph normally (surplus bot stays in emptyArea).`);
                }
                } // for surplusBot
                // Wave registration disabled – cleanup handled by _runCleanupPhase()
                if (surplusWaveMoves.length > 0) {
                    this.debugLog(`[EMPTYAREA] ${surplusWaveMoves.length} surplus bot(s) would be moved – deferred to cleanup phase.`);
                } // if surplusWaveMoves
            } // if surplusCount
        } // if emptyArea
        // ─────────────────────────────────────────────────────────────────

        // Surplus targets are already added to cluster_target – no cleanup needed here.

        this.debugLog(`[DEBUG stepMorph] checking areAllBotsHappy: targets=${this.cluster_target?.length ?? '?'} cells=${plannedCells?.length ?? '?'}`);
        if (this.areAllBotsHappy(this.cluster_target, plannedCells)) {
            this.debugLog(`[FLOW] All bots happy EARLY (before wave loop) – calling finishHandler with ${this.morphLog.waves.length} waves.`);
            this.log("All bots are happy! Morphing complete.");
            this.progress = 100;
            finishHandler(this.morphLog, true);
            return;
        }
        this.debugLog("[FLOW] Not all bots happy yet – entering wave loop.");
    ...
    // ====================================================================
    this.debugLog("[DEBUG stepMorph] Starting wave loop...");
    let waveIndex = 0;
    let globalRound = 0;
    const MAX_WAVES = 100; // Safety guard against infinite loops

    while (true) {
        if (waveIndex >= MAX_WAVES) {
            this.debugLog(`[DEBUG stepMorph] ERROR: exceeded max waves (${MAX_WAVES}), aborting!`);
            this.log(`ERROR: Morph exceeded max waves (${MAX_WAVES})`);
            finishHandler(this.morphLog, false);
            return;
        }
        ...
        if (this.areAllBotsHappy(this.cluster_target, plannedCells)) {
            this.debugLog("[FLOW] All bots happy after wave execution – breaking wave loop.");
            break;
        }
    } // wave loop

    this.debugLog(`[FLOW] Wave loop ended. Calling finishHandler with ${this.morphLog.waves.length} waves.`);
    ...
    // ─────────────────────────────────────────────────────────────────
    // _runCleanupPhase() – Second choosePair pass for surplus removal
    // After the main morph is complete, this creates cleanup targets
    // (free positions outside emptyArea with cluster contact) and runs
    // the wave loop with choosePair to move surplus bots out safely.
    // ─────────────────────────────────────────────────────────────────
    _runCleanupPhase(plannedCells, finishHandler) {
        const emptyArea = this.params?.emptyArea;
        const allBots = this.getAllBots(plannedCells);
        const targetBots = this.getAllBots(this.cluster_target);
        const targetPosSet = new Set(targetBots.map(t => `${t.x},${t.y},${t.z}`));

        // Find surplus bots: inside emptyArea, NOT at target positions
        const surplusBots = allBots.filter(b => {
            const x = Number(b.x), y = Number(b.y), z = Number(b.z);
            const inside = x >= emptyArea.x && x <= emptyArea.x2 &&
                           y >= emptyArea.y && y <= emptyArea.y2 &&
                           z >= emptyArea.z && z <= emptyArea.z2;
            return inside && !targetPosSet.has(`${x},${y},${z}`);
        });

        if (surplusBots.length === 0) {
            this.debugLog("[FLOW] No surplus bots to clean up.");
            finishHandler(this.morphLog, true);
            return;
        }

        this.debugLog(`[FLOW] Cleanup phase: ${surplusBots.length} surplus bot(s) to remove`);

        // Build occupied set
        const occupied = new Set();
        for (const b of allBots) occupied.add(`${b.x},${b.y},${b.z}`);
        for (const t of targetBots) occupied.add(`${t.x},${t.y},${t.z}`);

        // Find cleanup target for each surplus bot
        const cleanupTargets = [];
        for (const surplusBot of surplusBots) {
            let foundPos = null;
            // Scan for free positions outside emptyArea with cluster contact
            for (let r = 1; r <= 8 && !foundPos; r++) {
                for (let dx = -r; dx <= r && !foundPos; dx++) {
                    for (let dy = -r; dy <= r && !foundPos; dy++) {
                        for (let dz = -r; dz <= r && !foundPos; dz++) {
            ...
            if (foundPos) {
                cleanupTargets.push({ bot: surplusBot, target: foundPos });
                occupied.add(`${foundPos.x},${foundPos.y},${foundPos.z}`);
                this.debugLog(`[FLOW] Cleanup target for ${surplusBot.id}: (${surplusBot.x},${surplusBot.y},${surplusBot.z}) → (${foundPos.x},${foundPos.y},${foundPos.z})`);
            } else {
                this.debugLog(`[FLOW] No cleanup target found for ${surplusBot.id} @ (${surplusBot.x},${surplusBot.y},${surplusBot.z})`);
            }
        }

        if (cleanupTargets.length === 0) {
            this.debugLog("[FLOW] No cleanup targets found – skipping cleanup.");
            finishHandler(this.morphLog, true);
            return;
        }

        // Run choosePair for each cleanup target
        let cleanupRound = 0;
        const cleanupMaxRounds = 50;
        while (cleanupRound < cleanupMaxRounds) {
            cleanupRound++;
            const pathResult = {};
            const attemptedPairs = new Set();
            const reservedTargets = new Set();
            ...
            // Add cleanup targets as reserved (so choosePair knows about them)
            for (const ct of cleanupTargets) {
                const ck = `${ct.target.x},${ct.target.y},${ct.target.z}`;
                if (!this.getAllBots(plannedCells).some(b => b.x === ct.target.x && b.y === ct.target.y && b.z === ct.target.z)) {
                    // Not yet occupied – keep as free target
                }
            }

            // Temporarily add cleanup targets to cluster_target for choosePair
            const originalClusterTarget = this.cluster_target;
            // Create virtual targets: the surplus bots' current positions are already
            // "taken" by them, so choosePair will look for other donors... 
            // Actually, we need to trick choosePair into thinking cleanup targets
            // are real targets. Let's set cluster_target to include cleanup targets.

            const virtualTargets = cleanupTargets.map(ct => ({
                id: ct.bot.id,
                x: ct.target.x,
                y: ct.target.y,
                z: ct.target.z
            ...
            this.cluster_target = [...this.cluster_target, ...virtualTargets];

            const success = this.choosePair(
                plannedCells, attemptedPairs, reservedTargets,
                movedBotsInWave, new Set(), pathResult, []
            );

            // Restore original cluster_target
            this.cluster_target = originalClusterTarget;

            if (!success || !this.bot_start || !this.bot_target) {
                this.debugLog(`[FLOW] No more cleanup pairs found after ${cleanupRound} round(s).`);
                break;
            }

            // Register the move as a wave
            this.bot_target = {
                x: this.bot_target.x, y: this.bot_target.y, z: this.bot_target.z,
                vx: Number(this.bot_target.vx ?? 0), vy: Number(this.bot_target.vy ?? 0), vz: Number(this.bot_target.vz ?? 0)
            };
            const vkResult = pathResult.vkResult || null;
            if (vkResult && vkResult.ok && vkResult.states && vkResult.states.length >= 2) {
                this.wavecnt++;
                ...
                const waveMoves = [{
                    path: coordPath,
                    botStart: this.bot_start,
                    botTarget: this.bot_target,
                    botId: this.bot_id,
                    pathStates: vkResult.states
                }];
                this.registerMorphStepForExport(waveMoves, this.wavecnt);
                this.debugLog(`[FLOW] Cleanup wave ${this.wavecnt}: ${this.bot_id} → (${this.bot_target.x},${this.bot_target.y},${this.bot_target.z})`);
            }
        }

        this.debugLog(`[FLOW] Cleanup phase done – ${cleanupTargets.length} target(s), ${cleanupRound} round(s).`);
        finishHandler(this.morphLog, true);
    } // _runCleanupPhase()


    // ─────────────────────────────────────────────────────────────────
    // _removeSurplusBots() – Post-morph cleanup
    // After all target positions are filled, move surplus bots (bots inside
    // emptyArea that are NOT at target positions) to free positions outside
    // the emptyArea. Tries multiple candidate positions for each surplus bot
    // until a valid VK path is found. Registered moves are added to morphLog.
    // ─────────────────────────────────────────────────────────────────
    _removeSurplusBots(plannedCells) {
        const emptyArea = this.params?.emptyArea;
        if (!emptyArea || emptyArea.x === undefined) return;
        ...
        if (surplusBots.length === 0) {
            this.debugLog(`[REMOVE_UNUSED_SURPLUS] no additional surplus bots to remove.`);
            return;
        }

        // Only remove surplus bots if there are MORE bots in emptyArea than target positions.
        // If counts match, surplus bots should serve as donors (choosePair handles this).
        const allBotsInsideCount = allBots.filter(b => {
            const x = Number(b.x), y = Number(b.y), z = Number(b.z);
            return x >= emptyArea.x && x <= emptyArea.x2 &&
                   y >= emptyArea.y && y <= emptyArea.y2 &&
                   z >= emptyArea.z && z <= emptyArea.z2;
        }).length;
        if (allBotsInsideCount <= targetBots.length) {
            this.debugLog(`[REMOVE_UNUSED_SURPLUS] ${surplusBots.length} surplus bot(s) found but counts match (${allBotsInsideCount} ≤ ${targetBots.length}) – keeping as potential donors.`);
            return;
        }

        const surplusToRemoveCount = allBotsInsideCount - targetBots.length;
        this.debugLog(`[REMOVE_UNUSED_SURPLUS] begin – found ${surplusBots.length} surplus bot(s), removing ${surplusToRemoveCount} (${allBotsInsideCount} bots in emptyArea, ${targetBots.length} targets):`);
        for (const b of surplusBots) {
            this.debugLog(`[REMOVE_UNUSED_SURPLUS]   ${b.id} @ (${b.x},${b.y},${b.z})`);
        }

        let removedCount = 0;
        for (const surplusBot of surplusBots) {
            if (removedCount >= surplusToRemoveCount) {
                this.debugLog(`[REMOVE_UNUSED_SURPLUS]   ${surplusBot.id} @ (${surplusBot.x},${surplusBot.y},${surplusBot.z}) – keeping as potential donor (limit reached).`);
                continue;
            }
            // Collect all occupied positions for deduplication
            const occupied = new Set();
            for (const b of allBots) occupied.add(`${b.x},${b.y},${b.z}`);
            for (const t of targetBots) occupied.add(`${t.x},${t.y},${t.z}`);

            // Collect candidate positions outside emptyArea that have cluster contact.
            // Positions must be free AND adjacent to the cluster (hasContact) so the VK
            // path planner can find a reachable surface route from the surplus bot.
            // This avoids candidates floating in empty space with no VK connectivity.
            const candidates = [];
            for (let r = 1; r <= 6 && candidates.length < 15; r++) {
                for (let dx = -r; dx <= r && candidates.length < 15; dx++) {
                    for (let dy = -r; dy <= r && candidates.length < 15; dy++) {
                        for (let dz = -r; dz <= r && candidates.length < 15; dz++) {
                            const cx = Number(surplusBot.x) + dx;
                            const cy = Number(surplusBot.y) + dy;
                            const cz = Number(surplusBot.z) + dz;
                            // Skip positions inside emptyArea
                            if (cx >= emptyArea.x && cx <= emptyArea.x2 &&
                                cy >= emptyArea.y && cy <= emptyArea.y2 &&
                                cz >= emptyArea.z && cz <= emptyArea.z2) continue;
                            if (occupied.has(`${cx},${cy},${cz}`)) continue;
                            // Require cluster contact (at least one occupied neighbor)
                            // so the target is VK-reachable, not floating in space.
                            if (!this.hasContact(cx, cy, cz, plannedCells)) continue;
                            const ck = `${cx},${cy},${cz}`;
                            if (!candidates.some(c => c.x === cx && c.y === cy && c.z === cz)) {
            ...
            if (candidates.length === 0) {
                this.debugLog(`[REMOVE_UNUSED_SURPLUS] No free positions found for ${surplusBot.id}`);
                continue;
            }

            // Build path: surplus bot → outside position
            const surplusStart = {
                x: Number(surplusBot.x), y: Number(surplusBot.y), z: Number(surplusBot.z),
                vx: Number(surplusBot.vector_x ?? 0), vy: Number(surplusBot.vector_y ?? 0), vz: Number(surplusBot.vector_z ?? 1)
            };
            let pathFound = false;
            for (const candidate of candidates) {
                if (pathFound) break;
                this.debugLog(`[REMOVE_UNUSED_SURPLUS]   trying → (${candidate.x},${candidate.y},${candidate.z})`);
                // Pass start orientation as target orientation so the VK planner
                // doesn't waste time looking for neighbors in the (irrelevant) target structure.
                const surplusTarget = {
                    x: candidate.x, y: candidate.y, z: candidate.z,
                    vx: surplusStart.vx, vy: surplusStart.vy, vz: surplusStart.vz
                };
                const pathResult = {};
                const vkResult = this._buildWorldAndPlanPath(surplusStart, surplusTarget, plannedCells, surplusBot.id, null, pathResult);
                const planOk = vkResult && vkResult.ok && vkResult.states && vkResult.states.length >= 2;
                ...
                if (planOk) {
                    ...
                    // Just update plannedCells – actual wave registration
                    // happens in _runCleanupPhase() via choosePair.
                    this.debugLog(`[REMOVE_UNUSED_SURPLUS] Path found for ${surplusBot.id}: (${surplusBot.x},${surplusBot.y},${surplusBot.z}) → (${candidate.x},${candidate.y},${candidate.z}) states=${vk.states.length}`);
                    ...
                    occupied.add(`${candidate.x},${candidate.y},${candidate.z}`);
                    pathFound = true;
                } else {
                    // Log rejection reason from the VK result directly
                    let rejectReason = 'NO_RESULT';
                    if (vkResult) {
                        rejectReason = vkResult.error || 'VK_NO_PATH';
                        if (vkResult.states && vkResult.states.length < 2) {
                            rejectReason = 'TOO_FEW_STATES(' + vkResult.states.length + ')';
                        }
                    }
                    this.debugLog(`[REMOVE_UNUSED_SURPLUS]   → rejected: ${rejectReason}`);
                }
            }
            if (!pathFound) {
                this.debugLog(`[REMOVE_UNUSED_SURPLUS] No valid path for ${surplusBot.id} – ${candidates.length} candidates tried, all failed.`);
            }
        }
        this.debugLog(`[REMOVE_UNUSED_SURPLUS] end – ${removedCount} of ${surplusBots.length} surplus bot(s) removed.`);
    } // _removeSurplusBots()
    ...
    //
    // _logEmptyAreaClassification()
    // Logs surplus/need/donors_needed_outside counts for emptyArea debugging.
    // Does NOT modify the morph cycle – purely diagnostic.
    //
    _logEmptyAreaClassification() {
        const emptyArea = this.params?.emptyArea;
        if (!emptyArea || emptyArea.x === undefined) {
            this.debugLog("[EMPTYAREA] No emptyArea defined – skipping classification.\n");
            return;
        }
        const startBots = this.getAllBots(this.cells);
        const targetBots = this.getAllBots(this.cluster_target);

        // Count bot positions inside emptyArea
        const isInside = (bot) => {
            const x = Number(bot.x), y = Number(bot.y), z = Number(bot.z);
            return x >= emptyArea.x && x <= emptyArea.x2 &&
                   y >= emptyArea.y && y <= emptyArea.y2 &&
        ...
        const need = targetBots.length - startInside.length;
        const surplus = Math.max(0, startInside.length - targetBots.length);
        const donorsNeededOutside = Math.max(0, need);

        // Identify surplus candidates: bots inside emptyArea that are NOT in target
        const targetPosSet = new Set(targetBots.map(t => `${t.x},${t.y},${t.z}`));
        const surplusCandidates = startInside.filter(b => !targetPosSet.has(`${b.x},${b.y},${b.z}`));

        this.debugLog(`[EMPTYAREA] Classification:
  target structure bots:       ${targetBots.length}
  start bots inside emptyArea: ${startInside.length}
  need (target - startInside): ${need >= 0 ? '+' : ''}${need}
  surplus bots (to remove):    ${surplus}
  donors needed from outside:  ${donorsNeededOutside}
  all bots inside (IDs):      ${startInside.map(b => b.id ?? b.x+','+b.y+','+b.z).join(', ')}
  target positions inside:    ${targetInside.map(b => b.id ?? b.x+','+b.y+','+b.z).join(', ')}
  >>> surplus candidates (inside emptyArea, NOT in target) <<<
  ${surplusCandidates.length > 0 ? surplusCandidates.map(b => `    ${b.id} @ (${b.x},${b.y},${b.z})`).join('\n  ') : '    (none)'}
  emptyArea: (${emptyArea.x},${emptyArea.y},${emptyArea.z}) → (${emptyArea.x2},${emptyArea.y2},${emptyArea.z2})
`);
    } // _logEmptyAreaClassification()
    ...
    run(caller, finishHandler) {
        ...
        this.debugLog(`[FLOW] run() called – cells=${this.cells?.length ?? '?'} targets=${this.cluster_target?.length ?? '?'} wavecnt=${this.wavecnt} morphLog.waves=${this.morphLog?.waves?.length}`);

        // Log emptyArea classification (diagnostic only – no cycle impact)
        this._logEmptyAreaClassification();
...
} // class MorphVehicleKinematicsParallel2


module.exports = MorphVehicleKinematicsParallel2;
